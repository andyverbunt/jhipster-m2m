		...
		return {
			...
			controller: ['$scope', 
	            function($scope) {
					...
					var cleanHttpErrorListener = $rootScope.$on('m2mApp.httpError', function (event, httpResponse) {
					    var i;
					        ...
					        case 400:
					            if (httpResponse.data && httpResponse.data.fieldErrors) {
					                for (i = 0; i < httpResponse.data.fieldErrors.length; i++) {
					                    ...
					                    var fieldName = $translate.instant('m2mApp.' + fieldError.objectName + '.' + convertedField);
